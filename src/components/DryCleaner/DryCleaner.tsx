...
const DryCleanerList: React.FC = () => {
        ...
        cleanersData.map(async (cleaner) => {
          try {
            const bookingsResponse = await axios.get(
              `${import.meta.env.VITE_API_URL}/api/users/admin/get-drycleaner-booking-summary/${cleaner._id}`,
              { headers: { Authorization: `Bearer ${token}` } }
            );
            ...
            console.error(
              `Failed to fetch bookings for ${cleaner.shopname}`,
              error
            );
            return { totalBookings: 0, totalAmount: 0 };
          }
        })
      );

      const cleanersWithSummary = cleanersData.map((cleaner, i) => ({
        ...cleaner,
        ...bookingsSummary[i],
      }));
  ...
  const deleteDryCleaner = async (cleanerId: string) => {
    setIsDeleting(cleanerId);
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/users/admin/delete-dry-cleaner/${cleanerId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
        ...
        {dryCleaners.length === 0 ? (
          <div className="bg-white shadow rounded-lg p-8 text-center">
            ...
            <h3 className="mt-2 text-lg font-medium text-gray-900">
              No dry cleaners found
            </h3>
            <p className="mt-1 text-sm text-gray-500">
              There are currently no dry cleaners registered in the system.
            </p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <div className="mt-4 flex justify-between items-center border-t pt-4">
                    <button
                      onClick={() => setSelectedDryCleaner(cleaner)}
                    ...
                    <button
                      onClick={() => deleteDryCleaner(cleaner._id)}
                      disabled={isDeleting === cleaner._id}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="relative">
              <button
                onClick={() => setSelectedDryCleaner(null)}
              ...
              <div className="h-64 bg-gray-200 relative">
                ...
                selectedDryCleaner.shopimage.length > 0 ? (
                  <Slider
                    {...{
                      dots: true,
                      infinite: selectedDryCleaner.shopimage.length > 1,
                      speed: 500,
                      slidesToShow: 1,
                      slidesToScroll: 1,
                      arrows: true,
                  ...
                  >
                    {selectedDryCleaner.shopimage.map((img, idx) => (
                      <img
                        key={idx}
                        src={img}
                        alt={`${selectedDryCleaner.shopname} ${idx}`}
              ...
              <div className="p-6">
                <div className="flex items-start justify-between">
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">
                    {selectedDryCleaner.shopname}
                  </h2>
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ⭐ {selectedDryCleaner.rating}
                  </span>
                </div>

                <p className="text-gray-600 mb-4">{selectedDryCleaner.about}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-gray-700 mt-1">
                      ...
                      {selectedDryCleaner.phoneNumber}
                    </p>
                    <p className="text-gray-700 mt-1">
                      <span className="font-medium">Contact Person:</span>{' '}
                      {selectedDryCleaner.contactPerson}
                    </p>
                  </div>
